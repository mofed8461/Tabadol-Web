...
<title>Update School</title>
    ...
    <script language="javascript">
        function check()
        {
            var model  = document.updateform.city.value;

            if( model =="0")
            {
                alert("خطأ : يرجى اختيار مدينة ");
            }
        ...
        function goo(passid)
        {
            var result = confirm("هل انت متاكد من الرغبة بتحديث البيانات"+passid)
            // result = true  -> OK pressed
            // result = false -> Cancel pressed
            if(result == true)
            {
                window.location = "update2.php?id="+passid;
            }
        }
    </script>
    ...
    <form name="updateform" method="post" action="" enctype="multipart/form-data">
        <table align="center" style="margin-left: 40%;">
            <tr><td><select name="city" />
                ...
                <td>بحث حسب المدينة </td></tr>
            <tr><td></td><td><input type="button" value="بحث" onclick="check()"/> </td></tr>
    ...
    <div align="center">
        ...
        if( isset($_POST["city"]) )
        {
            ...
            include("connect.php");
            $query = $con ->query("select * from schools where city='$city'") or die("Error in query");
	...
	<div>عنوان المدرسة : $address</div>
	<div>المدينة : $city</div>
	 </td><td>  <input type='button' value='update' onclick='goo($id)' />   </td> </tr>";
            }
            echo "</table>";
        }
        ?>
